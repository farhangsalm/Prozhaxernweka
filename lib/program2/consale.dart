...
class csharbcon extends StatefulWidget {
  final Size size;
  final int itemIndex;
  final Consalee shar;
  ...
  @override
  _csharbconState createState() => _csharbconState();
}

class _csharbconState extends State<csharbcon> {
    ...
    return GestureDetector(
      onTap: () {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => ConColorKan(
                shar: widget.shar), // Replace YOUTBE with your desired page
          ),
        );
      },
      child: Container(
        ...
        child: Stack(
          children: [
            ...
            Padding(
              padding: const EdgeInsets.only(top: 144, left: 253),
              child: Container(
                height: 26,
                child: Text(
                  widget.shar.publisher,
...
class ConColorKan extends StatefulWidget {
  const ConColorKan({Key? key, required this.shar}) : super(key: key);
  final Consalee shar;

  @override
  State<ConColorKan> createState() => _ConColorKanState();
}

class _ConColorKanState extends State<ConColorKan> {
  ...
  void initState() {
    ...
    _controller = YoutubePlayerController(
      initialVideoId: widget.shar.video, // Replace with your video ID
  ...
  void copyText() {
    Clipboard.setData(ClipboardData(text: widget.shar.content));
  ...
  Future<void> _saveLikeState() async {
    ...
    await prefs.setBool('isLiked_${widget.shar.video}', isLiked);
  }
  ...
  Future<void> _loadLikeState() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      isLiked = prefs.getBool('isLiked_${widget.shar.video}') ?? false;
    });
  }
                            ...
                            RichText(
                              text: TextSpan(
                                text: isContentExpanded
                                    ? widget.shar.content
                                    : widget.shar.content.substring(0, 169) +
                                        '... ',
                                style: const TextStyle(
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        IconButton(
                                          onPressed: () {
                                            Share.share(widget.shar.content);
                                          },
                                          icon: const Icon(
                                            Icons.share,
                                            color: Colors.blue,
                                    ...
                                    StreamBuilder(
                                      stream:
                                          ref.child(widget.shar.video).onValue,
