...
class conpluse extends StatefulWidget {
  final Size size;
  final int itemIndex;
  final Con con;
  ...
  @override
  _conpluseState createState() => _conpluseState();
}

class _conpluseState extends State<conpluse> {
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) => ConColorKan(
                con: widget.con), // Replace YOUTBE with your desired page
          ),
        );
      },
      child: Container(
        ...
        child: Stack(
          children: [
            ...
            Padding(
              padding: const EdgeInsets.only(top: 144, left: 199),
              child: Container(
                height: 26,
                child: Text(
                  widget.con.publisher,
...
class ConColorKan extends StatefulWidget {
  const ConColorKan({Key? key, required this.con}) : super(key: key);
  final Con con;
...
class _ConColorKanState extends State<ConColorKan> {
  ...
  void initState() {
    ...
    _controller = YoutubePlayerController(
      initialVideoId: widget.con.video, // Replace with your video ID
  ...
  void copyText() {
    Clipboard.setData(ClipboardData(text: widget.con.content));
  ...
  Future<void> _saveLikeState() async {
    ...
    await prefs.setBool('isLiked_${widget.con.video}', isLiked);
  }
  ...
  Future<void> _loadLikeState() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      isLiked = prefs.getBool('isLiked_${widget.con.video}') ?? false;
    });
  }
                            ...
                            RichText(
                              text: TextSpan(
                                text: isContentExpanded
                                    ? widget.con.content
                                    : widget.con.content.substring(0, 169) +
                                        '... ',
                                style: const TextStyle(
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        IconButton(
                                          onPressed: () {
                                            Share.share(widget.con.content);
                                          },
                                          icon: const Icon(
                                            Icons.share,
                                            color: Colors.blue,
                                    ...
                                    StreamBuilder(
                                      stream:
                                          ref.child(widget.con.video).onValue,
