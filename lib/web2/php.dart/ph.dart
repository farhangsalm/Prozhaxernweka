// ignore_for_file: camel_case_types, library_private_types_in_public_api, prefer_const_constructors, prefer_interpolation_to_compose_strings, sort_child_properties_last
...
import 'package:flutter_application_1/web2/php.dart/phptab.dart';
import 'package:fluttertoast/fluttertoast.dart';
import 'package:share_plus/share_plus.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:youtube_player_flutter/youtube_player_flutter.dart';

class phpcrip extends StatefulWidget {
  final Size size;
  final int itemIndex;
  final Php php;
  ...
  @override
  _phpcripState createState() => _phpcripState();
}

class _phpcripState extends State<phpcrip> {
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => phpColorKan(php: widget.php),
          ),
        );
      },
      child: Container(
...
class phpColorKan extends StatefulWidget {
  const phpColorKan({Key? key, required this.php}) : super(key: key);
  final Php php;

  @override
  State<phpColorKan> createState() => _phpColorKanState();
}

class _phpColorKanState extends State<phpColorKan> {
  ...
  void initState() {
    ...
    _controller = YoutubePlayerController(
      initialVideoId: widget.php.video, // Replace with your video ID
  ...
  void copyText() {
    Clipboard.setData(ClipboardData(text: widget.php.content));
  ...
  Future<void> _saveLikeState() async {
    ...
    await prefs.setBool('isLiked_${widget.php.video}', isLiked);
  }
  ...
  Future<void> _loadLikeState() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      isLiked = prefs.getBool('isLiked_${widget.php.video}') ?? false;
    });
  }
                            ...
                            RichText(
                              text: TextSpan(
                                text: isContentExpanded
                                    ? widget.php.content
                                    : widget.php.content.substring(0, 169) +
                                        '... ',
                                style: const TextStyle(
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        IconButton(
                                          onPressed: () {
                                            Share.share(widget.php.content);
                                          },
                                          icon: const Icon(
                                            Icons.share,
                                            color: Colors.blue,
                                    ...
                                    StreamBuilder(
                                      stream:
                                          ref.child(widget.php.video).onValue,
...
class phpDetailsPage extends StatefulWidget {
  phpDetailsPage({
    Key? key,
  }) : super(key: key);

  @override
  _phpDetailsPageState createState() => _phpDetailsPageState();
}

class _phpDetailsPageState extends State<phpDetailsPage>
            ...
            children: [
              ...
              Expanded(
                child: TabBarView(
                  ...
                  children: [
                    ListView.builder(
                      itemCount: php.length,
                      itemBuilder: (context, index) => phpcrip(
                        itemIndex: index,
                        php: php[index],
                        size: size,
                      ),
