// ignore_for_file: prefer_const_constructors, prefer_const_literals_to_create_immutables, sort_child_properties_last, unused_element, camel_case_types, prefer_const_constructors_in_immutables, library_private_types_in_public_api, sized_box_for_whitespace, prefer_interpolation_to_compose_strings
import 'package:firebase_database/firebase_database.dart';
import 'package:flutter/gestures.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:flutter_application_1/program/bashijava/java2.dart';
import 'package:flutter_application_1/program/bashijava/java2tab.dart';
...
class Java3 extends StatefulWidget {
  final Size size;
  final int itemIndex;
  final Java1 jav;
  ...
  @override
  _Java3State createState() => _Java3State();
}

class _Java3State extends State<Java3> {
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => YOUTBE(jav: widget.jav),
          ),
        );
      },
      child: Container(
...
class YOUTBE extends StatefulWidget {
  const YOUTBE({Key? key, required this.jav}) : super(key: key);

  final Java1 jav;

  @override
  State<YOUTBE> createState() => _YOUTBEState();
}
...
class _YOUTBEState extends State<YOUTBE> {
  ...
  void initState() {
    ...
    _controller = YoutubePlayerController(
      initialVideoId: widget.jav.video, // Replace with your video ID
  ...
  void copyText() {
    Clipboard.setData(ClipboardData(text: widget.jav.content));
  ...
  Future<void> _saveLikeState() async {
    ...
    await prefs.setBool('isLiked_${widget.jav.video}', isLiked);
  }
  ...
  Future<void> _loadLikeState() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      isLiked = prefs.getBool('isLiked_${widget.jav.video}') ?? false;
    });
  }
                            ...
                            RichText(
                              text: TextSpan(
                                text: isContentExpanded
                                    ? widget.jav.content
                                    : widget.jav.content.substring(0, 169) +
                                        '... ',
                                style: const TextStyle(
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        IconButton(
                                          onPressed: () {
                                            Share.share(widget.jav.content);
                                          },
                                          icon: const Icon(
                                            Icons.share,
                                            color: Colors.blue,
                                    ...
                                    StreamBuilder(
                                      stream:
                                          ref.child(widget.jav.video).onValue,
...
class javaDetailsPage extends StatefulWidget {
  javaDetailsPage({
    Key? key,
  }) : super(key: key);

  @override
  _javaDetailsPageState createState() => _javaDetailsPageState();
}

class _javaDetailsPageState extends State<javaDetailsPage>
            ...
            children: [
              ...
              Expanded(
                child: TabBarView(
                  ...
                  children: [
                    ListView.builder(
                      itemCount: jav.length,
                      itemBuilder: (context, index) => Java3(
                        itemIndex: index,
                        jav: jav[index],
                        size: size,
                      ),
                    ...
                    ListView.builder(
                      itemCount: Varr.length,
                      itemBuilder: (context, index) => varjava(
                        itemIndex: index,
                        varr: Varr[index],
                        size: size,
                      ),
