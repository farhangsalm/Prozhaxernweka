// ignore_for_file: camel_case_types, library_private_types_in_public_api, prefer_const_constructors, sort_child_properties_last, unused_element, prefer_interpolation_to_compose_strings
...
class varjava extends StatefulWidget {
  final Size size;
  final int itemIndex;
  final Var varr;
  ...
  @override
  _varjavaState createState() => _varjavaState();
}

class _varjavaState extends State<varjava> {
    ...
    return GestureDetector(
      onTap: () {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => varColorKan(
                varr: widget.varr), // Replace YOUTBE with your desired page
          ),
        );
      },
      child: Container(
        ...
        child: Stack(
          children: [
            ...
            Padding(
              padding: const EdgeInsets.only(top: 144, left: 305),
              child: Container(
                height: 26,
                child: Text(
                  widget.varr.publisher,
...
class varColorKan extends StatefulWidget {
  const varColorKan({Key? key, required this.varr}) : super(key: key);
  final Var varr;

  @override
  State<varColorKan> createState() => _varColorKanState();
}

class _varColorKanState extends State<varColorKan> {
  ...
  void initState() {
    ...
    _controller = YoutubePlayerController(
      initialVideoId: widget.varr.video1, // Replace with your video ID
  ...
  void copyText() {
    Clipboard.setData(ClipboardData(text: widget.varr.content));
  ...
  Future<void> _saveLikeState() async {
    ...
    await prefs.setBool('isLiked_${widget.varr.video1}', isLiked);
  }
  ...
  Future<void> _loadLikeState() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      isLiked = prefs.getBool('isLiked_${widget.varr.video1}') ?? false;
    });
  }
                            ...
                            RichText(
                              text: TextSpan(
                                text: isContentExpanded
                                    ? widget.varr.content
                                    : widget.varr.content.substring(0, 169) +
                                        '... ',
                                style: const TextStyle(
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        IconButton(
                                          onPressed: () {
                                            Share.share(widget.varr.content);
                                          },
                                          icon: const Icon(
                                            Icons.share,
                                            color: Colors.blue,
                                    ...
                                    StreamBuilder(
                                      stream:
                                          ref.child(widget.varr.video1).onValue,
