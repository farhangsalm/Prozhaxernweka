...
class flatr1 extends StatefulWidget {
  final Size size;
  final int itemIndex;
  final Flatr flatr;
  ...
  _flatr1State createState() => _flatr1State();
}

// ignore: camel_case_types
class _flatr1State extends State<flatr1> {
    ...
    return GestureDetector(
      onTap: () {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => ColorKan(
                flatr: widget.flatr), // Replace YOUTBE with your desired page
          ),
        );
      },
...
class ColorKan extends StatefulWidget {
  const ColorKan({Key? key, required this.flatr}) : super(key: key);
  final Flatr flatr;

  @override
  State<ColorKan> createState() => _ColorKanState();
}

class _ColorKanState extends State<ColorKan> {
  ...
  void initState() {
    ...
    _controller = YoutubePlayerController(
      initialVideoId: widget.flatr.video1, // Replace with your video ID
  ...
  void copyText() {
    Clipboard.setData(ClipboardData(text: widget.flatr.content));
  ...
  Future<void> _saveLikeState() async {
    ...
    await prefs.setBool('isLiked_${widget.flatr.video1}', isLiked);
  }
  ...
  Future<void> _loadLikeState() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      isLiked = prefs.getBool('isLiked_${widget.flatr.video1}') ?? false;
    });
  }
                            ...
                            RichText(
                              text: TextSpan(
                                text: isContentExpanded
                                    ? widget.flatr.content
                                    : widget.flatr.content.substring(0, 169) +
                                        '... ',
                                style: const TextStyle(
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        IconButton(
                                          onPressed: () {
                                            Share.share(widget.flatr.content);
                                          },
                                          icon: const Icon(
                                            Icons.share,
                                            color: Colors.blue,
